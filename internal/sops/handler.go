...
func (h *Handler) List(c *gin.Context) {
	...
	if e != nil {
		httpx.WriteError(w, r, 500, httpx.Error{Code: "INTERNAL_ERROR", Message: "could not list SOPs"})
		return
	}
	httpx.Write(w, r, 200, items)
}
func (h *Handler) mutation(w http.ResponseWriter, r *http.Request, run func([]byte) (any, error)) {
...
func (h *Handler) Create(c *gin.Context) {
	w, r := c.Writer, c.Request
	h.mutation(w, r, func(raw []byte) (any, error) {
		var request CreateRequest
		decoder := json.NewDecoder(bytes.NewReader(raw))
		decoder.DisallowUnknownFields()
		if decoder.Decode(&request) != nil {
			return nil, errors.New("invalid SOP")
		}
...
func (h *Handler) Apply(c *gin.Context) {
	w, r := c.Writer, c.Request
	sopID, e := uuid.Parse(c.Param("id"))
	if e != nil {
		httpx.WriteError(w, r, 404, httpx.Error{Code: "NOT_FOUND", Message: "SOP not found"})
		return
	}
	h.mutation(w, r, func(raw []byte) (any, error) {
		var body struct {
		...
		count, e := h.service.Apply(r.Context(), sopID, body.TaskID)
		return map[string]any{"sop_id": sopID, "task_id": body.TaskID, "items_created": count}, e
	})
}
